...
void ControlOS::begin() {
  ...
  /*
   * Initialize the shared application SPI bus BEFORE TFT_eSPI.
   *
   * The previous build called SPI.begin() after the ST7789 was already
   * working. On this board that can reconfigure the SPI peripheral/pins
   * immediately after DISPLAY_READY and make all following TFT debug
   * output disappear.
   *
   * TFT_eSPI is initialized last so it owns the final display bus state.
   */
  SPI.begin(
      BoardPins::SpiSck,
      BoardPins::SpiMiso,
  ...
  RuntimeDebug::mark(
      ...
      "shared SPI initialized before TFT"
  );
  ...
  /*
   * Do NOT initialize the shared SPI peripheral again here.
   * TFT_eSPI has just established the known-good ST7789 bus state.
   */
  Wire.begin(
      BoardPins::I2cSda,
      BoardPins::I2cScl
