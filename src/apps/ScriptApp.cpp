...
void ScriptApp::ensureDefaults() {
  ...
  if (!LittleFS.exists("/scripts/circuit.cos")) {
    ...
    if (f) {
      f.println("CLEAR #000000"); f.println("COLOR #00FF50"); f.println("GOTO 24 85");
      f.println("FORWARD 55"); f.println("RIGHT 90"); f.println("FORWARD 28");
      f.println("LEFT 90"); f.println("FORWARD 72"); f.println("LEFT 45");
      f.println("FORWARD 35"); f.println("TEXT CONTROL//OS"); f.println("WAIT 900");
      f.close();
    }
  }
}
...
void ScriptApp::onSelect() {
  ...
  script_.attach(tft_);
  fs::FS& fs = onSd_[selected_] ? static_cast<fs::FS&>(SD) : static_cast<fs::FS&>(LittleFS);
...
void ScriptApp::tick(uint32_t nowMs) {
  if (runningView_) script_.tick(nowMs);
}

void ScriptApp::render(TFT_eSPI& tft) {
  tft_ = &tft;
  if (runningView_) {
    if (!script_.running()) Ui::footer(tft, script_.status(), "ENC: return");
    return;
  }
  tft.fillScreen(Theme::Bg);
  Ui::header(tft, "SCRIPT LAB", String(count_) + " scripts");
  if (!count_) {
    Ui::centered(tft, 76, "No /scripts/*.cos files", Theme::Muted, 2);
  } else {
    const uint8_t first = selected_ > 4 ? selected_ - 4 : 0;
    for (uint8_t row = 0; row < 5 && first + row < count_; ++row) {
      const uint8_t i = first + row;
      const int y = 32 + row * 21;
      if (i == selected_) Ui::panel(tft, 8, y, 304, 18, true);
      Ui::text(tft, 14, y + 2, names_[i], i == selected_ ? Theme::PrimaryBright : Theme::Text, 1);
    }
  }
